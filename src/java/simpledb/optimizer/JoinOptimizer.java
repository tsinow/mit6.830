...
//                simple heuristic 0.3*card1*card2
                card = (card1 * card2 * 3) / 10;
//                make sure bigger than non-primary equality join
...
//    need to optimize
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        for (LogicalJoinNode node : joins) {
            if (!stats.containsKey(node.t1Alias))
                throw new ParsingException("the stats missing the table " + node.t1Alias);
            if (!stats.containsKey(node.t2Alias))
                throw new ParsingException("the stats missing the table " + node.t2Alias);
            if (!filterSelectivities.containsKey(node.t1Alias))
                throw new ParsingException("the filterSelectivity missing the table " + node.t1Alias);
            if (!filterSelectivities.containsKey(node.t2Alias))
                throw new ParsingException("the filterSelectivity missing the table " + node.t2Alias);
        }
        PlanCache planCache = new PlanCache();
        for (int len = 1; len <= joins.size(); len++) {
            Set<Set<LogicalJoinNode>> fixedLengthJoin = enumerateSubsets(joins, len);
            for (Set<LogicalJoinNode> joinSet : fixedLengthJoin) {
                CostCard bestPlan = null;
                double minCost = Double.POSITIVE_INFINITY;
                for (LogicalJoinNode node : joinSet) {
                    CostCard costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, node, joinSet, minCost, planCache);
                    if (costCard == null) continue;
                    if (bestPlan == null || minCost > costCard.cost) {
                        bestPlan = costCard;
                        minCost = costCard.cost;
                    }
                }
                if (bestPlan != null) planCache.addPlan(joinSet, bestPlan.cost, bestPlan.card, bestPlan.plan);
            }
        }
